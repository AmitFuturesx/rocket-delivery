...
(function () {
    ...
    function render(q) {
      ...
      if (q.listedPrice) {
        ...
        if (note) note.textContent = 'המחיר לפי מחירון החברה לאזור הזה, לפני תוספות. ' +
          'הוא עשוי לעלות או לרדת בהתאם לדרישות שלכם — שעות לילה, סופי שבוע ' +
          'או המתנה בכתובת.';
      } else {
        $('#r-service-label').textContent = SERVICE_NAMES[q.service];
        $('#r-service').textContent = shekel(q.net);
      ...
      if (!q.listedPrice && note) {
        note.textContent = 'זהו מחיר משוער. הוא עשוי לעלות או לרדת בהתאם ' +
          'לדרישות שלכם — שעות לילה, סופי שבוע, המתנה בכתובת או טיפול חריג. ' +
          'המחיר הסופי נסגר איתכם בוואטסאפ לפני שהשליח יוצא.';
      }
  ...
  if (cb) {
    cb.addEventListener('submit', function (e) {
      ...
      window.location.href = 'mailto:Rocketdeliveryil12@gmail.com' +
